...
public class Com2DataTypeToManagedDataTypeConverterTests
{
    ...
    [Fact]
    public void Com2DataTypeToManagedDataTypeConverter_ConvertNativeToManaged_WithoutProperty_ReturnsValue()
    {
        // Arrange
        Com2ColorConverter converter = new();
        int oleColor = ColorTranslator.ToOle(Color.Yellow);
        VARIANT nativeValue = (VARIANT)oleColor;
    ...
    [Fact]
    public void Com2DataTypeToManagedDataTypeConverter_MultipleConversions_Consistent()
    {
        // Arrange
        Com2ColorConverter converter = new();
        Color[] testColors = [Color.Red, Color.Green, Color.Blue, Color.White];
        bool cancelSet = false;

        // Act & Assert
        foreach (Color testColor in testColors)
        {
            VARIANT nativeValue = converter.ConvertManagedToNative(testColor, s_stubDescriptor, ref cancelSet);
            object result = converter.ConvertNativeToManaged(nativeValue, s_stubDescriptor);
    ...
    [Fact]
    public void Com2DataTypeToManagedDataTypeConverter_ColorConverter_HandlesKnownColors()
    {
        // Arrange
        Com2ColorConverter converter = new();
        bool cancelSet = false;

        // Act & Assert - Test various known colors
        Color[] knownColors = 
        [
            Color.Transparent,
            Color.AliceBlue,
        ...
        foreach (Color knownColor in knownColors)
        {
            int oleColor = ColorTranslator.ToOle(knownColor);
            VARIANT nativeValue = (VARIANT)oleColor;
            object result = converter.ConvertNativeToManaged(nativeValue, s_stubDescriptor);
    ...
    [Fact]
    public void Com2DataTypeToManagedDataTypeConverter_ColorConverter_StressTest()
    {
        // Arrange
        Com2ColorConverter converter = new();
        bool cancelSet = false;

        // Act & Assert - Test with multiple conversions using valid color values
        for (int i = 0; i < 50; i++)
        {
            ...
            int green = (i * 7) % 256;
            int blue = (i * 11) % 256;
            Color testColor = Color.FromArgb(red, green, blue);
